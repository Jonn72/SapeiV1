//Paso 1 video
$('#cbxVideo').change(function () {
    if ($(this).is(':checked')) {
        $("#btnCurso").attr("disabled", false);
        $(this).remove();
    }
});
$("#btnCurso").click(function () {
    $.ajax({
        url: '../../../Estudiante/ModificaEstadoSS',
        type: 'POST',
        dataType: 'json',
        data: {piEstado:1, psValores:""},
    })
        .done(function (data) {
...
$("#btnModalidad").click(function () {
    ...
    var turno = $("#cboTurno").val();

    $.ajax({
        url: '../../../Estudiante/ModificaEstadoSS',
        type: 'POST',
        dataType: 'json',
        data: { piEstado: 1, psValores: modalidad + "|" + turno },
    })
        .done(function (data) {
            if (typeof (data.Success) !== "undefined" && !data.Success) {
                ...
                $('#BodyPrincipal').load('../../../../Estudiante/ProcesoSS2');
                setTimeout(1000);
