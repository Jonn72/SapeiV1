$(document).ready(function () {
    $('#datatable-buttons tbody').on('click', 'a.btn-primary', function (event) {
        ...
        $('#ModalReporteBimestral1').modal('show');//abrir modal    
    ...
    $("#frmReporteBimestral1").submit(function (evento) {
        var calificacion = $("#txtCalificacion").val();
        var control = $("#Controltxt").val();
        $.ajax({
            url: '../../../Vinculacion/ValidarBimestre1SS',
            type: 'POST',
            dataType: 'json',
            data: {
              ...
              .done(function (data) {
                  if (typeof (data.Success) !== "undefined" && !data.Success) {
                      ...
                      $('#ModalReporteBimestral1').modal('hide'); // cerrar
                      $('body').removeClass('modal-open');
                      $('.modal-backdrop').remove();
                      $('#BodyPrincipal').load('../../../../Vinculacion/Bimestre1SS');
                  }
              })
              .fail(function (data) {
