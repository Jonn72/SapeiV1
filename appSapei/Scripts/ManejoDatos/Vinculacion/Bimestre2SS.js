$(document).ready(function () {
    $('#datatable-buttons tbody').on('click', 'a.btn-primary', function (event) {
        ...
        $('#ModalReporteBimestral2').modal('show');//abrir modal    
...
$("#frmReporteBimestral2").submit(function (evento) {
    var calificacion = $("#txtCalificacion").val();
    var control = $("#Controltxt").val();
    $.ajax({
        url: '../../../Vinculacion/ValidarBimestre2SS',
        type: 'POST',
        dataType: 'json',
        data: {
          ...
          .done(function (data) {
              if (typeof (data.Success) !== "undefined" && !data.Success) {
                  ...
                  $('#ModalReporteBimestral2').modal('hide'); // cerrar
                  $('body').removeClass('modal-open');
                  $('.modal-backdrop').remove();
                  $('#BodyPrincipal').load('../../../../Vinculacion/Bimestre2SS');
              }
          })
          .fail(function (data) {
