...
namespace Sapei
{
	...
	/// </summary>
	public partial class RF_historial_pagos
	{
		#region variables
		#endregion
		#region Propiedades
		...
		public string GuardaRegistros(string psCadena, string psNombreArchivo)
		{
			...
			foreach (string lsRegistro in lsCadena.Trim().Split(new char[2] { '\n', '\r' }))
			{
				...
				lsControl = lasRegistro[10].Trim();
				lsControl = lsControl.Substring(lsControl.Length - 10, 10);
				lsControl = lsControl.LTrim('0');
			...
			_oSistema.Conexion.InsertBulkCopy(loTabla, "rf_historial_pagos");
			return null;
		}
		...
		public DataTable RegresaRegistroPagos(string psPeriodo)
		{
			StringBuilder lsQuery = new StringBuilder();
			lsQuery.Append("SELECT  no_de_control, importe, referencia_1 ,observaciones, fecha_registro");
			lsQuery.AppendFormat(" from {0} A", RutaTabla);
		...
		public DataTable RegresaRegistroPagosReinscripcion(string psPeriodo)
		{
			...
			using (var loConexion = new ManejaConexion(_oSistema.Conexion))
			{
				...
				loDt.Load(_oSistema.Conexion.EjecutaProcedimientoAlmacenadoDataReader("pac_rf_pagos_reinscripcion", loParametros));
			}
