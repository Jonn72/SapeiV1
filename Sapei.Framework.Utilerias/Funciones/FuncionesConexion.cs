...
{
	class FuncionesConexion : IDisposable
	{

		public void Dispose()
		{
			DisconnectFromShare(@"\\192.168.9.181\HistoriaAcademica\", false);
		}

		public FuncionesConexion(string psServidor, NetworkCredential poCredenciales)
		{
			try
			{

				DisconnectFromShare(psServidor, true);

				//Disconnect in case we are currently connected with our credentials;
				ConnectToShare(psServidor, poCredenciales.UserName, poCredenciales.Password); //Connect with the new credentials
			}
